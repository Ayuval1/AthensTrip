...
import { getResponse } from '../lib/chatLogic.js'
...
export default function ChatAgent() {
  ...
  async function send(text) {
    ...
    setMessages(prev => [...prev, { role: 'user', parts: [{ text: q }], links: [] }])
    setInput('')
    setLoading(true)

    await new Promise(r => setTimeout(r, 350))

    const { text: responseText, links } = getResponse(q)
    setMessages(prev => [...prev, { role: 'model', parts: [{ text: responseText }], links }])
    setLoading(false)
  }
                ...
                <div className="flex flex-wrap gap-1.5 justify-end">
                  {msg.links.map((link, j) => (
                    <a
                      ...
                      style={{ background: '#C9A84C', color: 'white', textDecoration: 'none' }}
                    >
                      {link.label}
                    </a>
                  ))}
